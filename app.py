...
RESULT_PROFILES = {
    "proctor": {
        ...
        "one_line": (
            "당신은 자신을 정의로운 사람이라고 쉽게 믿지 않습니다. 자신의 잘못과 모순을 알고 있기에, 누군가를 비판하거나 진실을 말하려 할 때조차 ‘내가 그럴 자격이 있는가’를 먼저 묻게 됩니다.
            하지만 침묵이 계속될수록, 아무것도 하지 않는 선택 역시 하나의 선택이라는 사실과 마주하게 됩니다. 결국 당신에게 중요한 것은 완벽한 사람이 되는 것이 아니라, 자신의 잘못을 인정한 뒤에도 무엇을 옳다고 선택할 것인가입니다."
        ),
        "strength": (
            "끝까지 남아 있는 양심과 자기 판단"
        ),
        ...
        "watch": (
            "프록터가 누구를 지키려 하는지만 보지 마세요.
그가 왜 스스로를 쉽게 용서하지 못하는지, 그리고 언제부터 바깥의 마녀사냥과 싸우는 일이 자기 자신과의 싸움으로 바뀌는지 지켜보세요."
    ...
    "hale": {
        ...
        "character": "헤일",
        "summary": (
            "당신은 불안과 소문이 커질수록 감정보다 근거를 확인하려 하고, 충분한 정보를 바탕으로 판단하려 합니다.
처음에는 자신이 믿는 기준과 절차가 문제를 바로잡을 수 있다고 생각해 적극적으로 개입하지만, 그 판단 역시 틀릴 수 있다는 가능성을 닫아두지는 않습니다.
새로운 사실이 기존의 확신과 충돌한다면 체면이나 일관성을 지키기보다, 자신의 오류를 인정하고 잘못된 흐름을 바로잡는 쪽을 선택합니다."
        ),
        "one_line": (
            "처음에는 제도와 기준을 믿지만, "
        ...
        "watch": (
            "헤일이 무엇을 근거로 사람들을 판단하는지, 그리고 자신이 믿었던 기준에 의문을 품기 시작하는 순간 무엇이 달라지는지 지켜보세요.
확신을 버리는 일이 그에게 후퇴인지, 또 다른 책임의 시작인지 생각해 보세요."
        ),
